...
describe("API Test from SanityTest JSON", () => {
  ...
  it("Validates API sanityTest Responses", () => {
  ...
  cy.request({
    ...
    if (response.status === 503 && retries > 0) {
        ...
        const actualResults = response.body.data.map(item => ({
          ...
          const expected = testCase["Expected Results"];
    ...
    expect(matchesUrl, `${actualItem.url} - URL should contain one of the expected patterns ${expected.url}  `).to.be.true;
